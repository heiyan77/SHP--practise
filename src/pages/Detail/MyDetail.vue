...
export default {
  ...
  methods: {
    ...
    async addShopcar(){
      ...
      // 上面的代码：调用仓库中的addOrUpdateShopCart函数，这个函数加上了async，返回的一定是一个Promise
      // 2.这次请求要么成功要么失败
      try {// 成功
        await this.$store.dispatch("addOrUpdateShopCart",{skuId:this.$route.params.skuid,skuNum:this.skuNum});

        // 3.路由跳转
        // 4.路由跳转时需要将产品的信息带给下一级的路由组件
        // 一些简单的数据skuNum,通过query形式给路由组件传过去
        // 产品的信息数据(比较复杂的数据),通过会话存储(不持久化,会话结束数据在消失)
