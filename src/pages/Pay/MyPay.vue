  ...
  methods: {
    ...
    async open() {
      ...
      this.$alert(`<img src=${url} />`, "微信支付", {
        ...
        confirmButtonText: "支付已完成",// 确定按钮的文本内容
        showCancelButton:true,// 是否显示取消按钮
        center:true,// 是否居中布局
        showClose:false,// 是否显示右上角关闭按钮
        dangerouslyUseHTMLString:true, // 是否将 message第一个参数 作为 HTML 片段处理
        beforeClose:(action, instance, done) => {
          ...
          if(action == 'cancel'){
            ...
            // 判断是否支付
            if(this.code == 200){// 为了测试方便可以不判断this.code
              // 清除定时器
              clearInterval(this.timer);
              this.timer = null;
              // 关闭弹出框
