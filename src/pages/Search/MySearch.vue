...
export default {
  ...
  methods: {
    getData() {
      ...
      // 把这次请求封装成一个函数，在需要的时候调用即可
      this.$store.dispatch("getSearchList", this.searchParams);
    },
    // 删除分类的名字
    removeCategoryName() {
      // 把带给服务器的参数置空了，但还要向服务器发请求
      // 如果服务器参数说明了是可有可无的，那么如果属性值为空的字符串还是会把相应的字段带给服务器
      ...
      // 严谨：本意是删除query，如果路径当中出现params不应该删除，当前这个字段不会带给服务器
    ...
    // 排序的操作
    changeOrder(flag){// flag代表用户点击的是综合(1)还是价格(2)
      // 获取最开始的状态
